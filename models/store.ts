...
export function addBookingToStore(store: Store, data: NewBooking): Booking {
	if (hasDateOverlap(store.bookings, data.startDate, data.endDate)) {
		throw new Error(
			`Date conflict: the store already has a booking that overlaps with ${data.startDate} → ${data.endDate}.`,
		)
	}
